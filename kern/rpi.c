...
void uart_config_and_enable(size_t line) {
  ...
  switch (line) {
    ...
    case MARKLIN: baud_ival = 1258; baud_fval = 0; break;
    default: return;
  }
...
void uart_putc(size_t line, unsigned char c) {
  switch(line) {
    case CONSOLE: push(&CHAR_OUT_Q_CONSOLE, c); break;
    case MARKLIN: push(&CHAR_OUT_Q_MARKLIN, c); break;
  };
  try_uart_out();
}
...
bool uart_non_blocking_putc(size_t line, unsigned char c) {
  if (UART_REG(line, UART_FR) & UART_FR_TXFF) return false;
  UART_REG(line, UART_DR) = c;
  return true;
}
